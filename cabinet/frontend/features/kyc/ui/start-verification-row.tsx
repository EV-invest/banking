  ...
  return (
    <>
      ...
      <div className="flex flex-col py-3.5">
        ...
        {/* The row has no gap of its own, so the message brings its own margin. The live
            region stays OUTSIDE the handover: `role="status"` mounted in the same tick as
            its own text is unreliably announced (see ./verification-controls). */}
 ...
 * you" is a promise with no handle on it — this is the handle, and it is the same one the
 * `unavailable` outcome offers, so a reader who is stuck reaches the same mailbox either way.
 */
