...
//!   works for a single-instance dev/CI hub (mirrors the cabinet BFF's session store).
...
/// destructive rotation, so the fallible steps of a refresh (the user lookup) can
