...
// `gen-api` app + pre-commit hook). The BFF reaches the hub over gRPC, so these are
// TYPES ONLY — snake_case property names matching the proto-loader `keepCase` shape —
// not an HTTP client. Output is committed so the app type-checks without the backend.
